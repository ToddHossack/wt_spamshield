...
class tx_veguestbook_pi1 extends tslib_pibase {
	...
	function init($conf) {
		...
		$this->config['obligationfields'].= $this->conf['obligationFields'];
	...
	function getPageBrowser($markerArray) {
		...
		if ($newsCount > $begin_at + $this->config['limit']) {
			...
			$params = array($this->prefixId.'[pointer]' => $next);

			if ($this->for_tt_news) {
				$params = array_merge($params, $this->tt_news);
			}

			$next_link = $this->pi_linkTP($this->pi_getLL('pi_list_browseresults_next', 'Next >'), $params, $this->allowCaching);
		...
		if ($begin_at) {
			...
			$params = array($this->prefixId.'[pointer]' => $prev);

			if ($this->for_tt_news) {
				$params = array_merge($params, $this->tt_news);
			}

			$prev_link = $this->pi_linkTP($this->pi_getLL('pi_list_browseresults_prev', '< Previous'), $params, $this->allowCaching);
		...
		for ($i = $firstPage ; $i < $lastPage; $i++) {
			if (($begin_at >= $i * $this->config['limit']) && ($begin_at < $i * $this->config['limit'] + $this->config['limit'])) {
				...
				$params = array($this->prefixId.'[pointer]' => $i);

				if ($this->for_tt_news) {
					$params = array_merge($params, $this->tt_news);
				}

				$link = $this->pi_linkTP($this->local_cObj->stdWrap($item, $this->conf['pageBrowser.']['pagelink_stdWrap.']) , $params, $this->allowCaching) . ' ';
	...
	*/
	...
	function checkForm() {
		...
		if (t3lib_extMgm::isLoaded('captcha') && $this->config['captcha'] == 'captcha') {
			...
			if ($captchaStr != $this->postvars['captcha_response']) {
				$error .= '<li>'.ucfirst($this->pi_getLL('form_captcha_response'))." (".$this->pi_getLL('form_invalid_field').")</li>\n";
			}
		}	
